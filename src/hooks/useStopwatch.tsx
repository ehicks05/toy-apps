...
export const useStopwatch = () => {
	...
	useEffect(() => {
		function startTimer() {
			interval.current = window.setInterval(() => setNow(new Date()), TIME_STEP);
		}
	...
	const totalTime = now && startedAt ? differenceInMilliseconds(now, startedAt) : 0;
	...
	return {
		...
		reset,
		addLap,
		formatTime: (ms: number) => formatTime(ms, forceShowMinutes, forceShowHours),
	};
};
