	...
	days.forEach((day, i) => {
		const eventsInDay = eventsInWeek.filter(
			(event) =>
				...
				(i === 0 &&
					Temporal.PlainDate.compare(event.start.toPlainDate(), day.toPlainDate()) <
						0),
		);
		...
		eventsInDay.forEach((event) => {
			...
			dailyOffsets.forEach((offset, i) => {
				dailyOffsets[i] = event.dayMask[i] ? offsetMovingForward : dailyOffsets[i];
			});
		});
	});
...
export const Calendar = ({ date: _date }: Props) => {
	...
	const weeks = chunk(days, cols).map((week, i) => {
